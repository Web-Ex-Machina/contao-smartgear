...
class Rsce extends Block implements BlockInterface
{
    ...
    public function install()
    {
        try {
            ...
            $this->objFiles->rcopy('public/bundles/wemsmartgear/rsce_files', 'templates/rsce');
            ...
            return [
                'toastr' => [
                    'status' => 'success', 'msg' => "L'installation des templates RSCE a été effectuée avec succès.",
                ], 'callbacks' => [
                    0 => [
    ...
    public function remove()
    {
        try {
            ...
            return [
                'toastr' => [
                    'status' => 'success', 'msg' => 'La désinstallation des templates RSCE a été effectuée avec succès.',
                ], 'callbacks' => [
                    0 => [
                        'method' => 'refreshBlock', 'args' => ['block-'.$this->type.'-'.$this->module],
                    ],
    ...
    protected function shouldBeUpdated()
    {
        try {
            clearstatcache();
            $arrSrcFolder = scandir(TL_ROOT.'/public/bundles/wemsmartgear/rsce_files');
            ...
            foreach ($arrSrcFolder as $rsce) {
                ...
                if (md5_file(TL_ROOT.'/public/bundles/wemsmartgear/rsce_files/'.$rsce) !== md5_file(TL_ROOT.'/templates/rsce/'.$rsce)) {
