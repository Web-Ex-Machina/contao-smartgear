...
class Faq extends Block implements BlockInterface
{
    ...
    public function install()
    {
        ...
        if (!file_exists('templates/smartgear/mod_faqpage.html5')) {
            ...
            $objFiles->copy('public/bundles/wemsmartgear/templates_files/mod_faqpage.html5', 'templates/smartgear/mod_faqpage.html5');
        }
        ...
        Util::updateConfig([
            'sgFAQInstall' => 1, 'sgFAQ' => $objFAQ->id, 'sgFAQModule' => $objModule->id, 'sgFAQPage' => $intPage,
        ]);

        // And return an explicit status with some instructions
        return [
            'toastr' => [
                'status' => 'success', 'msg' => 'La configuration du module a été effectuée avec succès.',
            ], 'callbacks' => [
                0 => [
    ...
    public function remove()
    {
        ...
        Util::updateConfig([
            'sgFAQInstall' => '', 'sgFAQ' => '', 'sgFAQModule' => '', 'sgFAQPage' => '',
        ]);

        // And return an explicit status with some instructions
        return [
            'toastr' => [
                'status' => 'success', 'msg' => 'La suppression du module a été effectuée avec succès.',
            ], 'callbacks' => [
                0 => [
                    'method' => 'refreshBlock', 'args' => ['block-'.$this->type.'-'.$this->module],
                ],
