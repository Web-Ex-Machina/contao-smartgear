...
{
    /** @var string */
    protected string $strTemplate = 'be_wem_sg_block_events_dashboard';
    ...
    protected function getFilledTemplate(): FrontendTemplate
    {
        ...
        $objTemplate->mode = $config->getSgEvents()->getSgMode();
        $objTemplate->installComplete = $config->getSgEvents()->getSgInstallComplete();
        $objTemplate->installLocked = $config->getSgInstallLocked();

        if (!$config->getSgInstallLocked()) {
            if (!$config->getSgEvents()->getSgInstallComplete()) {
