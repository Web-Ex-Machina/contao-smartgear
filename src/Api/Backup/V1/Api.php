...
class Api
{
    ...
    public function delete(string $backupName)
    {
        try {
            if (!$this->backupManager->delete($backupName)) {
                throw new \Exception($this->translator->trans('WEMSG.BACKUPMANAGER.API.messageDeleteError', ['backup' => $backupName]));
            }
        } catch (\Exception $e) {
            throw new \Exception($e->getMessage());
        }

        return json_encode(['message' => $this->translator->trans('WEMSG.BACKUPMANAGER.API.messageDeleteSuccess', ['backup' => $backupName])]);
    }
    ...
    public function restore(string $backupName)
    {
        ...
        return json_encode(['message' => $this->translator->trans('WEMSG.BACKUPMANAGER.API.messageRestoreSuccess', ['backup' => $backupName])]);
    }
